...
- **Video clipping (`videoMetadata`) never worked** — since 1.x. The offsets were nested inside `fileData`; the API wants them beside it on the part and rejected every clipped request (`Unknown name "videoMetadata" at … file_data`). The mocked unit test asserted the wrong shape; a wire test through the real SDK now pins the right one. Found by the live battery.
- **Veo `negativePrompt` and `personGeneration` are checked against what the API accepts** (live battery, 2026-09-23): Veo 3.1 Lite rejects `negativePrompt` (Fast and Standard accept it); text-to-video and extension accept only `personGeneration: 'allow_all'`, image-to-video `allow_all` or `allow_adult`, and `dont_allow` is rejected in every mode. These were server 400s; they are now `ValidationError`s before any request, downgradable with `capabilityValidation: 'warn'` (Google restricts some regions to `allow_adult`). `VeoFeatures` gains an optional `negativePrompt`; new export `VEO_PERSON_GENERATION_BY_MODE`.

- **A missing or expired video file is reported as one.** Google answers `generateFromVideo` on a file that no longer exists with **403** `PERMISSION_DENIED` ("…or it may not exist"), not 404 (observed live 2026-09-23), so 1.x's 404-only hint never fired and the caller was told it was an authentication failure. The 404 keeps 1.x's "Video file not found… may have expired" error; the 403 — whose wording Google uses for a file that is gone *and* for one this key cannot see (e.g. another project's) — says "not found or not accessible with this API key" and names both. Both are classified `USER_ACTIONABLE`. The check matches Google's current wording; if Google rewords it, the error falls back to `AUTH`. `deleteVideoFile()` treats a 403 like a 404 (already gone).
...
- CLI: a run with no mode or no `--prompt` now says which (`Error: no mode selected…`, `Error: --prompt is required…`) before the help text; 1.x printed only the help. A multi-prompt run that fails part-way reports how many prompts completed and which one failed (the batch still stops at the first failure).
...
- **Image URL downloads could reach internal addresses (SSRF).** 1.x validated the URL once with string prefixes, which let through IPv4-mapped IPv6 in hex (`https://[::ffff:a9fe:a9fe]/` is the cloud metadata service 169.254.169.254 — `new URL()` normalises the dotted form to this, so only the dotted form was ever caught), all of `fc00::/7` except the prefixes `fc00:`/`fd00:`, carrier-grade NAT, every DNS answer after the first, a different answer at download time, and redirects (followed unchecked, including https → http). Now: CIDR ranges in a `net.BlockList`, which matches IPv4-mapped IPv6 in any form, plus the IPv6 blocks that embed IPv4 (IPv4-compatible `::/96`, 6to4, Teredo, NAT64 — the first three found by the ship security review after the first version of the fix missed them); every resolved address checked, **at connect time** for each connection the download makes; each redirect checked before it is followed (HTTPS only, no internal host). The README's security section previously claimed IPv4-mapped forms were blocked; it now describes what is checked. Found by the ship review; bfl-api has the same prefix-based check.
