...
- **Image URL downloads could reach internal addresses (SSRF).** 1.x validated the URL once with string prefixes, which let through IPv4-mapped IPv6 in hex (`https://[::ffff:a9fe:a9fe]/` is the cloud metadata service 169.254.169.254 — `new URL()` normalises the dotted form to this, so only the dotted form was ever caught), all of `fc00::/7` except the prefixes `fc00:`/`fd00:`, carrier-grade NAT, every DNS answer after the first, a different answer at download time, and redirects (followed unchecked, including https → http). Now: CIDR ranges in a `net.BlockList`, which matches IPv4-mapped IPv6 in any form; every resolved address checked, **at connect time** for each connection the download makes; each redirect checked before it is followed (HTTPS only, no internal host). The README's security section previously claimed IPv4-mapped forms were blocked; it now describes what is checked. Found by the ship review; bfl-api has the same prefix-based check.
