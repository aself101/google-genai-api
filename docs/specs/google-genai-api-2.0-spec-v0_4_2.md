...
| V16 (L) | **Through the built 2.0 package on SDK 2.24** (not raw SDK calls): neutral prompt, `9:16` on `gemini-3.1-flash-image` → height > width; `imageSize: '2K'` → larger than the 1K default (P3b correction: `imageSize` is a resolution tier, not a long-edge cap — 2048×2048 at 1:1, 2816×1536 at default framing) | a square or landscape result blocks release — the headline Fixed entry is verified on the shipped artifact, not on 1.30 (run #2 A9) |
